...
module.exports = {
	index: (req,res)=>{
		console.log("in players.index");
		Player.find({}).sort('-score').exec((err,players)=>{
			if(err){
				console.log("how do you even get this to error?")
				res.json(err);
			}else{
				console.log("gottem");
				res.json(players);
	...
	create: (req,res)=>{
		...
		Player.findOne({username:req.body.player.username}).exec((err,player)=>{
			if(player){
				...
				console.log("player not in db, creating new document");
